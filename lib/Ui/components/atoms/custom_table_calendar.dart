...
class CustomTableCalendar extends StatefulWidget {
  const CustomTableCalendar(
      {super.key, required this.callback, this.paddingTop});
...
class _CustomTableCalendarState extends State<CustomTableCalendar> {
  ...
  void showCalendar({required DateTime init, required DateTime last}) {
    ...
    showCalendarDatePicker2Dialog(
      ...
      if (value == null) {
        ...
        if (value.length > 1) {
          setRange(startValue: value.first, endValue: value.last);
        }
  ...
  Widget build(BuildContext context) {
    ...
    return Expanded(
      ...
      child: Padding(
        padding:
            EdgeInsets.only(top: widget.paddingTop ?? 16, right: 16, left: 16),
        child: Column(children: [
          ...
          TableCalendar(
              ...
              onPageChanged: (currentFocusedDay) =>
                  pageChanged(newFocusedDay: currentFocusedDay),
              onCalendarCreated: (pageController) =>
                  _pageController = pageController,
              onRangeSelected: (start, end, currentFocusedDay) {
                setRange(startValue: start, endValue: end);
